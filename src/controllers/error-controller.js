...
const globalErrorHandler = (err, req, res, next) => {
    ...
    if (config.nodeenv === 'production') {
        ...
        if (err.inner?.message === 'No authorization token was found')
            error = handleUnauthorizedError('Se necesita la credencial para ingresar.', 401);
        if (err.inner?.name === 'TokenExpiredError') error = handleJWTExpiredError('El token ha expirado.', 401);
        if (err.inner?.name === 'JsonWebTokenError') error = handleJsonWebTokenError('El token es inválido.', 401);
