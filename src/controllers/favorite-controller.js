...
exports.addFavorite = catchAsync(async (req, res, next) => {
  ...
  if (exists)
    return next(new AppError('This pokemon is already in your favorites', 409));
  ...
  const favoriteData = await cache(cachedFavoriteKey, async () => {
    ...
    if (!response.ok)
      throw new AppError('Error al obtener el Pokémon', response.status);
  ...
  return res
    .status(201)
    .json({ message: 'Pokémon añadido a favoritos', favorite: newFavorite });
});
...
exports.getFavorites = catchAsync(async (req, res, next) => {
  ...
  const favorites = await cache(cachedFavoritesKey, async () => {
    const favs = await Favorite.find({ user: userId, isActive: true }).select(
      '-__v'
    );
    return favs;
  });
